...
public class HorizontalSlider : Widget, ILinkable
{
    ...
    public override void Update()
    {
        ...
        Rectangle hitArea = new(Location.X - Size, Location.Y - Size, Length + Size * 2, Size * 2);
        if (hitArea.Contains(Gui.MousePosition) || dragging)
        {
            ...
            if (Gui.LMouseDown)
            {
                // Update
                State = 2;
                float val = Length <= 0 ? 0 : Math.Clamp((Gui.MousePosition.X - Location.X) / (float)Length, 0, 1);
                dragging = true;
